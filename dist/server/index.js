...
function shell(path, title, subtitle, body) {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${title}</title><style>${style}</style></head><body><div class="shell"><aside class="side"><a href="/" class="brand"><i>◈</i><span style="display:inline-block;max-width:145px;font-size:11px;line-height:1.35;vertical-align:middle">LABORATORY QUALITY<br>THINKING MODEL (LQTM)</span></a><nav class="nav"><small>Workspace</small><a class="${path === "/" || path === "/dashboard" ? "on" : ""}" href="/">◫ &nbsp; Dashboard</a><a class="${path === "/audit" ? "on" : ""}" href="/audit">▣ &nbsp; Audit</a><a class="${path.startsWith("/capa") ? "on" : ""}" href="/capa/2">⚠ &nbsp; CAPA</a></nav><div class="foot">● Internal audit<br><small>Draft · Jul 2026</small></div></aside><main class="main"><header class="head"><div><span class="kicker">ISO 9001:2015</span><h1>${title}</h1><p>${subtitle}</p></div></header>${body}</main></div><script>${stateScriptV2}</script></body></html>`;
}
function audit() {
  let qs = questions
    .map(
